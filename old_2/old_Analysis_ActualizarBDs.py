        ...
        with conn:
            for ticker in indice:
                ...
                if not permiteActualizar(logger):
                    ...
                    last_id = conn.execute(query).fetchone[0]
                    ...
                    # 'Date' es el index:
                    if not hist.empty and (hist['id'].max() != last_id):
...
def permiteActualizar(logger):
    try:
        ...
        inicio = horarioUTC.replace(hour=4, minute=0, second=0, microsecond=0)
        fin = horarioUTC.replace(hour=10, minute=30, second=0, microsecond=0)
